...
@author: Kim
@rewrite: HQL
function: Point Cloud Augmentation with 1 parameter
"""
...
class Batch_PointWOLF(object):
    ...
    def __call__(self, b_pos):
        ...
        output :
            pos([B,N,3]) : original pointcloud
            pos_new([B,N,3]) : Pointcloud augmneted by PointWOLF
        """
        M = self.num_anchor  # (Mx3)
        B, N, C = b_pos.shape  # (N)
        result_aug = np.zeros((B, N, C))
        # print('input pos shape: ', b_pos.shape)

        for i in range(0, B):
            pos = b_pos[i]
            # print('after slice pos shape: ', pos.shape)
            if self.sample_type == 'random':
                idx = np.random.choice(N, M)  # (M)
            elif self.sample_type == 'fps':
                idx = self.fps(pos, M)  # (M)
